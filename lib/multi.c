                                 ...
                                 bool use_wakeup)
{
...
#ifdef USE_WINSOCK
        if(WSAEventSelect(s, multi->wsa_event, wsa_events.lNetworkEvents) != 0)
  ...
  for(i = 0; i < extra_nfds; i++) {
    ...
    if(extra_fds[i].events & CURL_WAIT_POLLPRI) {
      wsa_events.lNetworkEvents |= FD_OOB;
    ...
    if(WSAEventSelect(extra_fds[i].fd, multi->wsa_event,
                      wsa_events.lNetworkEvents) != 0)
      return CURLM_INTERNAL_ERROR;
#else
        ...
        while(data) {
          ...
          for(i = 0; i < MAX_SOCKSPEREASYHANDLE; i++) {
            if(bitmap & (GETSOCK_READSOCK(i) | GETSOCK_WRITESOCK(i))) {
              ...
              if(ret && !timeout_ms && !wsa_events.lNetworkEvents) {
                if((bitmap & GETSOCK_READSOCK(i)) &&
