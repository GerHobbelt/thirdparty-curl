  ...
  */
  ...
  while(backend->encdata_offset > 0 && sspi_status == SEC_E_OK &&
    ...
    if(sspi_status == SEC_E_OK || sspi_status == SEC_I_RENEGOTIATE ||
	  ...
	  // (e.g. arxiv.org) so we only mention this once per request/response.
	  if (!data->state.reported_need_more_data) {
        SCH_DEV(infof(data, "schannel: failed to decrypt data, need more data (this is a benign issue; no user action required)"));
