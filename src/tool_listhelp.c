...
const struct helptxt helptext[] = {
  ...
  {"    --create-dirs",
   ...
   CURLHELP_OUTPUT},
  {"    --output-path-mimics-url",
   "Write the content/file to a relative path that's constructed from the source URL itself. Remixes the server-specified filename into this path when either `remote-name-all` or `remote-header-name` options have been specified as well. Implicitly sets `create-dirs` alongside.",
   CURLHELP_OUTPUT},
  ...
  {"    --proxy-pinnedpubkey <hashes>",
   ...
   CURLHELP_PROXY | CURLHELP_TLS},
  {"    --proxy-safe-auth",
   "Disable proxy clear password authentication mechanisms",
   CURLHELP_PROXY | CURLHELP_AUTH},
  ...
  {"    --retry-max-time <seconds>",
   ...
   CURLHELP_CURL | CURLHELP_TIMEOUT},
  {"    --safe-auth",
   "Disable clear text password authentication mechanisms",
   CURLHELP_AUTH},
  {"    --sanitize-with-extreme-prejudice",
   "Enable local output filename+path sanitization: while the file path may be constructed from a user-specified output path, the source URL and/or the Content-Type reported by the responding server, the sanitization of the emerging file path will ensure the downloaded data/content is stored in a file that has a legal/sane path for any modern platform",
   CURLHELP_AUTH},
