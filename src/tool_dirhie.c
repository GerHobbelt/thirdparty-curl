...
CURLcode create_dir_hierarchy(const char *outfile, struct GlobalConfig *global)
{
  ...
  /* [GHo] DO NOT allow strtok() here since this MAY be used threaded as in:
     multiple cURL library/API invocations being executed in a multithreaded 
	 master application. */
  tempdir = outdup;
	  ...
	  if(dlen)
        ...
        if(outdup == tempdir) {
          ...
          /* Skip creating a drive's current directory.
             ...
             erroneously.
             e.g. if outfile is X:\foo\bar\filename then do not mkdir X:
