                        ...
                        CURLSH *share, const char *filename)
{
  ...
  Curl_dyn_init(&buf, MAX_SSLS_LINE);
  ...
  while(my_get_line(fp, &buf, &error)) {
    ...
    *c = '\0';
    r = Curl_base64_decode(line, &shmac, &shmac_len);
    if(r) {
    ...
    r = Curl_base64_decode(line, &sdata, &sdata_len);
    if(r) {
                              ...
                              const char *alpn, size_t earlydata_max)
{
  ...
  r = Curl_base64_encode((const char *)shmac, shmac_len, &enc, &enc_len);
  if(r)
    goto out;
  r = CURLE_WRITE_ERROR;
  ...
  if(EOF == fputc(':', ctx->fp))
    goto out;
  curl_free(enc);
  r = Curl_base64_encode((const char *)sdata, sdata_len, &enc, &enc_len);
  if(r)
    goto out;
  r = CURLE_WRITE_ERROR;
  if(enc_len != fwrite(enc, 1, enc_len, ctx->fp))
