...
struct OperationConfig {
  ...
  BIT(rm_partial);                /* on error, remove partially written output
                                     files */
  BIT(skip_existing);
  BIT(sanitize_with_extreme_prejudice); /* Sanitize URLs with extreme prejudice, i.e.
...
struct GlobalConfig {
  ...
  char *help_category;            /* The help category, heap-allocated if set */
