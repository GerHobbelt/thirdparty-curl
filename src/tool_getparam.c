                                ...
                                struct OperationConfig *config)
{
  ...
  if(!err) {
    if(config->query) {
      CURLcode result = Curl_dyn_addf(&dyn, "%s&%s", config->query, query);
      free(query);
      if(result)
        err = PARAM_NO_MEM;
      else {
                               ...
                               struct OperationConfig *config)
{
  ...
  if(Curl_dyn_len(&config->postdata)) {
    /* skip separator append for --json */
    if(!err && (cmd != C_JSON)  &&
       Curl_dyn_addn(&config->postdata, "&", 1))
      err = PARAM_NO_MEM;
  }

  if(!err && Curl_dyn_addn(&config->postdata, postdata, size))
    err = PARAM_NO_MEM;

  Curl_safefree(postdata);

  config->postfields = Curl_dyn_ptr(&config->postdata);
  return err;
}
  ...
  *usedarg = FALSE; /* default is that we don't use the arg */
  ...
  do {
    ...
    case C_XATTR: /* --xattr */
      ...
      case C_TCP_MAXSEG: /* --tcp-maxseg */
        err = str2unummax(&config->tcp_maxseg, nextarg, 65535);
        if(err)
          return err;
        break;

      case C_PROGRESS_PCT: /* --progress-percent */
        global->progressmode = CURL_PROGRESS_PERCENT;
        break;

      case C_SAFE_AUTH: /* --safe-auth, disable clear text password authentication. */
        config->safeauth &= ~CURLSAFE_AUTH;
        if(toggle)
          config->safeauth |= CURLSAFE_AUTH;
        break;

      case C_PROXY_SAFE_AUTH: /* --proxy-safe-auth, disable clear password authentication. */
        config->safeauth &= ~CURLSAFE_PROXYAUTH;
        if(toggle)
          config->safeauth |= CURLSAFE_PROXYAUTH;
        break;

	  case C_SANITIZE_WXP: /* --sanitize-with-extreme-prejudice */
        config->sanitize_with_extreme_prejudice = toggle;
        break;

	  case C_OUTPUT_PATH_MIMICS_URL: /* --output-path-mimics-url */
